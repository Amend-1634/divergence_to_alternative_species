...
include { REMAP_ANC_READS; REMOVE_C_TO_T_FORWARD_G_TO_A_REVERSE } from './remapping_ancient_reads_remove_CT_GA.nf'
...
workflow {
	...
	// remove the C-to-T on forward strand and G-to-A on reverse strand
	REMOVE_C_TO_T_FORWARD_G_TO_A_REVERSE( anc_bam.merge(mod_fa) )

	//(2)ancient bam with the modern reference
	anc_bam_mod_fa = REMOVE_C_TO_T_FORWARD_G_TO_A_REVERSE.out
...
process COMPUTE_MUTATION {
    ...
    container 'docker://biopython/biopython'
    publishDir params.results_dir, mode: "copy"
